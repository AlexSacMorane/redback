...
TO DO add download / figure free energy / links with repositories
...
The `Phase-Field` module of MOOSE should be used to solve this problem.

#### Mesh

The first called block is the Mesh block.
For this example, the domain is a 2D square meshed with a regular grid.

In this example, the mesh is created by using a mesh generator from MOOSE. 
The quality of the simulation can be increased with the use of a more sophisticated mesh, involving refinement of the mesh close to the interface of the grain.
...
#### Variables
Once the mesh is defined, a phase variable ($\eta_1$) is generated in the domain. This variable is equal to 1 if the point corresponds to the given grain, else it is equal to 0. 
The pore space is determined by complementarity.
The interface between the solid and the pore phases is represented by $0<\eta_1<1$.
An additional variable $c$ is generated in the domain to represent the concentration of the solute.
In this example, the initial conditions of the variables are generated by a MOOSE function that reads .txt files (generated during the preparation of the PF).
...
In this case, the system is divided into two equations: Eq. 1 applied on the phase variable $\eta_1$, and Eq. 3 applied on the variable $c$.
...
As described in Eq. 1, a Allen-Cahn equation, applied to the variable $\eta_1$, is solved to predict the dissolution/precipitation of the solid phase. 
This equation can be divided into three kernels:

- `TimeDerivative`: $\frac{\partial \eta_1}{\partial t}$ 
- `AllenCahn`: $L\;\frac{\partial\left( f_{loc} + E \right)}{\partial \eta_1}$
- `ACInterface`: $L\cdot \kappa \; \nabla^2 \eta_1$
...
- `CoefCoupledTimeDerivative`: $\alpha_{\eta_1}\;\frac{\partial\eta_1}{\partial t}$. It is worth noting that this kernel uses the variable $\eta_1$ with the input `v` even if it is applied to the equation applied to the variable $c$
- `ACInterface`: $L_c\cdot \kappa_c \; \nabla^2 c$


In the same note, additional parameters are introduced `L_eta`, `g_eta_1`, `kappa_eta`, `kappa_c`, and `L_c` defined in the block `Materials` (see below).
...
The function `GenericConstantMaterial` assigns a constant value to a property in the entire domain, while the functions `DerivativeParsedMaterial` and `GenericFunctionMaterial` generate heterogeneous fields for the property. 

Concerning the function `DerivativeParsedMaterial`, it is divided into several main inputs:

- `property_name`: the name of the property, used for instance in the block `Kernels`
- `expression`: the expression of the free energy $f_{loc}+E$ of the material 
- `coupled_variables`: the definition of the variables used in the `expression` of the property. These variables can be a material or a variable of the problem
...
In particular, the simulated time is determined by the stopping conditions (here `end_time`), and the quality of the results depends on the tolerance applied to the residual (here `l_tol`, `l_abs_tol`, `nl_rel_tol`, `nl_abs_tol`).
...
Different types of output are generated at the initial and final conditions (see the command `execute_on`) :

- a .e file with the command `exodus` to obtain the field of the variables.
- in the same vein, vtk files are generated to be read by python scripts
